...
data_labels <- read.spss("data/Homicide_report.sav", to.data.frame=TRUE)
data_only <- read.spss("data/Homicide_report.sav", to.data.frame=TRUE, use.value.labels=F)


new_labels <- select(data_labels,
                     ID, CNTYFIPS, Ori, State, Agency, AGENC0,
